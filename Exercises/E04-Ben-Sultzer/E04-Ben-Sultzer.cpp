...
void update() {
	...
	if (ballIsActive == false) {
		...
		if (((ballPos[0] >= (barPos[0] - (barSize / 2))) &&
			(ballPos[0] <= (barPos[0] + (barSize / 2)))) &&
			((ballPos[1] - radius) <= barPos[1])) {
			velocity_y = abs(velocity_y); // When the ball hits the bar, the y-velocity should stay the same and become positive. The x-velocity is unaffected.
		}
